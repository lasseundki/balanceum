...
export default function Analytics() {
  ...
  const pieData = Object.entries(catTotals)
    .map(([id, value]) => ({ name: catMap[id]?.name ?? 'Sonstiges', value }))
  ...
  return (
    <div className="px-4 pt-4 pb-nav space-y-4">
      ...
      <div className="grid grid-cols-3 gap-2">
        {[
          { label: 'Einnahmen', val: totalIncome, color: 'text-success' },
          { label: 'Ausgaben', val: totalExpense, color: 'text-error' },
          { label: 'Bilanz', val: totalIncome - totalExpense, color: totalIncome - totalExpense >= 0 ? 'text-success' : 'text-error' },
        ].map(({ label, val, color }) => (
      ...
      {outliers.length > 0 && (
        <div className="flex items-start gap-3 bg-warning-light border border-warning rounded-lg p-3">
          ...
          <div>
            <p className="text-xs font-semibold text-text">Ausreißer erkannt</p>
            <p className="text-xs text-text-secondary">
              {outliers.map(o => o.name).join(', ')} über 150% des Monatsdurchschnitts
            </p>
          </div>
        </div>
      )}
      ...
      {extraordinary > 0 && (
        <div className="bg-surface border border-border rounded-lg p-3 flex justify-between items-center">
          <span className="text-sm text-text-secondary">⚡ Außergewöhnliche Ausgaben</span>
      ...
      <div className="bg-surface border border-border rounded-xl p-4">
        <div className="flex justify-between items-center mb-1">
          <h2 className="font-heading text-base font-semibold text-text">Monatsübersicht</h2>
          {selectedMonth !== null && (
            <button onClick={() => setSelectedMonth(null)} className="text-xs text-accent font-medium">Zurücksetzen</button>
          )}
        </div>
        <p className="text-xs text-text-muted mb-3 font-heading italic">Tippe auf einen Monat für Details</p>
        {loading ? (
          <div className="h-40 flex items-center justify-center text-text-muted text-sm">Laden...</div>
        ) : (
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={monthData} onClick={(e: unknown) => {
              ...
              />
              <Bar dataKey="income" name="Einnahmen" fill="#7BA89B" radius={[4, 4, 0, 0]} />
              <Bar dataKey="expense" name="Ausgaben" fill="#B87B72" radius={[4, 4, 0, 0]}
      ...
      {pieData.length > 0 && (
        <div className="bg-surface border border-border rounded-xl p-4">
          <h2 className="font-heading text-base font-semibold text-text mb-1">
            Ausgaben nach Kategorie {selectedMonth !== null ? `· ${MONTHS[selectedMonth]}` : '· Gesamt'}
          </h2>
