  ...
  return (
    <div className="px-4 pt-4 pb-nav space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="text-xs text-text-muted uppercase tracking-wide font-semibold">Willkommen zurück</p>
      ...
      <div className="bg-accent rounded-xl p-5 text-text-inverse shadow-lg">
        ...
        <p className="text-xs opacity-75 uppercase tracking-wider mb-1">Saldo</p>
        ...
        <div className="flex gap-4 mt-4 pt-4 border-t border-white/20">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-xs opacity-70">Einnahmen</p>
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-xs opacity-70">Ausgaben</p>
      ...
      {hasExtraordinary && (
        <div className="flex items-center gap-3 bg-warning-light border border-warning rounded-lg p-3">
          ...
          <div>
            <p className="text-xs font-semibold text-text">Außergewöhnliche Ausgaben</p>
            <p className="text-xs text-text-secondary">{fmt(extraordinary)} diesen Monat</p>
          </div>
        </div>
      )}
      ...
      {topCats.length > 0 && (
        <div className="bg-surface border border-border rounded-xl p-4">
          <h2 className="font-heading text-base font-semibold text-text mb-3">Top Ausgaben</h2>
              ...
              return (
                <div key={catId}>
                  <div className="flex justify-between items-center mb-1">
                    <div className="flex items-center gap-2">
                      <span>{cat?.icon ?? '📌'}</span>
                      <span className="text-sm font-medium text-text">{cat?.name ?? 'Unbekannt'}</span>
                    </div>
      ...
      {recent.length > 0 && (
        <div className="bg-surface border border-border rounded-xl p-4">
          <h2 className="font-heading text-base font-semibold text-text mb-3">Letzte Buchungen</h2>
      ...
      {!loading && transactions.length === 0 && (
        <div className="text-center py-12 text-text-muted">
          ...
          <p className="font-heading text-lg font-medium text-text-secondary">Noch keine Buchungen</p>
          <p className="text-sm mt-1">Tippe auf + um zu starten</p>
        </div>
      )}
    </div>
  )
